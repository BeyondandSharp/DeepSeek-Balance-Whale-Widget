...
function dshwStartOnce() {
  ...
  dshwStarted = true
  // 初始化失败必须看得见：这里原来是空 catch，而 dshwInit 是「整个挂件 = 一个函数」
  // （15000+ 行全在它里面），任何一处抛错 —— 尤其是只在 Gecko 上出现的 —— 都会被完全
  // 吞掉：不打日志、不重试，表现成「挂件半死 / 不出现，但控制台一片干净」，无从排查。
  // 现在把原始异常与堆栈打出来；仍然不重试，因为半初始化的 DOM/监听重跑会重复挂载。
  try { dshwInit() } catch (err) {
    try { console.error('[小鲸鱼] 初始化失败，挂件不可用，请把这条报错一并反馈：', (err && err.stack) || err) } catch (err2) {}
  }
}
...
function bubbleDefaultQueue() {
  ...
  // 原来这个 return 之后还留着 248 行「旧默认体」死代码（历届默认泡泡队列的快照）：
  // Firefox/V8 每次解析都报 unreachable code after return statement，既刷控制台、
  // 又把真正的报错盖住；而且它全靠「不可达」才没生效，谁改它都是白改。已删除。
